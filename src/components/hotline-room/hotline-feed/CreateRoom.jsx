...
import { useRTCStore } from "../../../hotline-room-engine/webrtc/store/useRTCStore";
import { useVoiceStore } from "../../../hotline-room-engine/webrtc/store/useVoiceStore";
import { api } from "../../../api";


const CreateRoom = ({ conn }) => {
  const [roomdesc, setRoomDesc] = useState("");
  const [enableAutoSpeaker, setAutoSpeaker] = useState(false);
  const [isPrivate, setPrivate] = useState(false);
  const [enableRoomChat, setRoomChat] = useState(false);
  const [enableHandRaise, setHandRaise] = useState(false);
  ...
  const createRoomMutation = useMutation({
    mutationFn: async (params) => {
      const { data } = await api.post("/api/room/create", params);
      return data;
    },
            ...
            <Toggle
              ...
              onClick={e => {
                if (selectedToggles.includes(category)) {
                  setSelectedToggles(
                    selectedToggles.filter(toggle => toggle !== index+1)
                  );
                } else {
                  if (selectedToggles.length <= 4) {
                    setSelectedToggles([...selectedToggles, index + 1]);
                  } else {
                  }
                }
