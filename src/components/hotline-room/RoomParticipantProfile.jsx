...
const RoomParticipantProfile = ({
  ...
  const { data: participant, isLoading: participantLoading } =
    useQuery({
      ...
      queryFn: async () => {
        const { data } = await api.get(`/api/profile/${participantId}`);
        return data;
      },
    });
  ...
  const {
    ...
    const { data } = await api.get(`/api/room/ban/${room.roomId}`);
    return data;
  });
  ...
  const followMutation = useMutation({
    mutationFn: async (params) => {
      params.isAFollow
        ? await api.post("/api/profile/follow", {
            userToFollow: params.userToFollow,
          })
        : await api.delete(`/api/profile/unfollow/${params.userToUnFollow}`);
    },
  ...
  const banMutation = useMutation({
    mutationFn: async (params) => {
      ...
      params.isBan
        ? await api.post(
            `/api/room/ban/${room.roomId}?userId=${params.userId}&banType=${params.banType}`
          )
        : await api.delete(
            `/api/room/unban/${room.roomId}?userId=${params.userId}&banType=${params.banType}`
          );
    },
