...
const ReportComponent = ({ data }) => {
  ...
  const [generateConfirmationVisible, setGenerateConfirmationVisible] =
    useState(false);
          ...
          {data?.priorityDistribution?.map((priority, index) => (
            <div key={index} className="border rounded p-4">
              ...
              <p className="text-gray-600">
                Priority Count: {priority.priority_count}
              </p>
            </div>
          ))}
        </div>
        ...
        <div className="grid grid-cols-2 gap-4">
          {data?.issuesByCategory?.map((category, index) => (
            <div key={index} className="border rounded p-4">
              ...
              <p className="text-gray-600">
                Issue Count: {category.issue_count}
              </p>
            </div>
          ))}
        </div>
  ...
  const renderRecentIssueActivity = () => {
    ...
    return (
      <div>
        <h2 className="text-xl font-semibold mb-4">Recent Issue Activity</h2>
        <div className="grid grid-cols-1 gap-4">
          {data?.recentIssueActivity?.map((issue, index) => (
            <div key={index} className="border rounded p-4">
              ...
              <p className="text-slate-500">Description: {issue.description}</p>
              <p className="text-slate-500">Status: {issue.status}</p>
      ...
      <div className="flex justify-between mb-4">
        ...
        <div className="flex gap-2 items-center">
          ...
          <Bot
            ...
            width={30}
            height={30}
            onClick={() => openGenerateConfirmation()}
          />
      ...
      <AppDialog
        ...
        content={
          <>
            <AiReport
              stat={data}
              closeGenerateConfirmation={closeGenerateConfirmation}
            />
          </>
        }
      ></AppDialog>
